...
class ResearchController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            // ✅ Allow ALL authenticated users (admins & normal users) to fetch research data
            $courseId = $request->input('course_id');
    
            $researches = Research::with(['course.category']) // Load category via course
            ...
            return DataTables::of($researches)
                ->addColumn('course_name', fn($research) => $research->course->name ?? 'N/A')
                ->addColumn('category_name', fn($research) => $research->course->category->name ?? 'N/A') // Add category
                ->addColumn('action', fn($research) => '<a href="/storage/' . $research->file_path . '" target="_blank" class="btn btn-primary btn-sm">View</a>')
                ->rawColumns(['action'])
                ->toJson();
        }
    
        // ❌ Admins only: Show the research management page
        if (auth()->user()->role === 'admin') {
            $courses = Courses::with('category')->get();
            return view('research.research', compact('courses'));
        }
    
        // ❌ Block normal users from accessing the research management page
